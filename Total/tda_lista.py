...
def insertar(lista, dato):
    ...
    if (lista.inicio is None) or (lista.inicio.info.usuario > dato.usuario):
        nodo.sig = lista.inicio
        lista.inicio = nodo
    else:
        ant = lista.inicio
        act = lista.inicio.sig
        while(act is not None and act.info.usuario.usuario < dato.usuario):
            ant = ant.sig
            act = act.sig
        nodo.sig = act
        ant.sig = nodo
...
def buscar(lista, buscado):
    aux = lista.inicio
    while(aux is not None and aux.info.usuario != buscado):
        aux = aux.sig
    return aux.info
